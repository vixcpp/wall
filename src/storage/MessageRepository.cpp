...
namespace wall::storage
{
  ...
  {
    /**
     * @brief Build a domain message from a database row.
     */
  ...
  {
    return sqlite_.with_connection(
        [&id](auto &conn) -> std::optional<wall::domain::Message>
        {
          auto stmt = conn->prepare(
